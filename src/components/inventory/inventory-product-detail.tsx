...
export function InventoryProductDetail({ product, initialSkuId, initialChannelId, onClose }: InventoryProductDetailProps) {
  ...
  const channelRows = getChannelInventoryBySku(selectedSku.id).filter((row) => row.channelType !== 'CENTER');
  const operation = SKU_OPERATION_DATA[selectedSku.id];
  const selectedChannel = channelRows.find((row) => row.id === selectedChannelId) ?? channelRows.find((row) => row.health === 'SHORTAGE') ?? channelRows[0];
  const requestDestination = selectedChannel;
  const sourceCandidates = channelRows
    .filter((row) => row.id !== requestDestination?.id)
    .sort((left, right) => (right.availableStock - right.safetyStock) - (left.availableStock - left.safetyStock));
  const requestSource = sourceCandidates.find((row) => row.id === requestSourceId) ?? sourceCandidates[0];
  const requestMode = requestSource && requestDestination && requestSource.fulfillmentCenter === requestDestination.fulfillmentCenter ? '재고 재할당' : 'RT';
  const transferableQuantity = Math.max(0, (requestSource?.availableStock ?? 0) - (requestSource?.safetyStock ?? 0));
  ...
  const selectedChannelLots = baseLots.map((lot, index) => {
    ...
    return {
      ...
      location: `${selectedChannel.channelName} · ${selectedChannel.fulfillmentCenter}`,
    };
  });
  ...
  const totalDailySales = channelRows.reduce((sum, row) => sum + row.dailySales, 0);
  const totalForecast = channelRows.reduce((sum, row) => sum + row.forecast14Days, 0);
  const salesStock = channelRows.reduce((sum, row) => sum + row.stock, 0);
  ...
  const forecastData = forecastDays.map((day) => {
    const remaining = Math.max(0, Math.round(salesStock - dailySales * day));
    const accumulatedLoss = Math.round(((salesStock + remaining) / 2) * day * selectedSku.sellingPrice * 0.00018 / 10000);
    return { label: day === 0 ? '현재' : `D+${day}`, remaining, accumulatedLoss };
  });

  const startAiStrategy = () => {
    const query = new URLSearchParams({ productId: product.id, skuId: selectedSku.id, channelId: selectedChannel.id });
  ...
  const openTransferRequest = () => {
    ...
    setRequestQuantity(Math.min(20, Math.max(1, (preferredSource?.availableStock ?? 1) - (preferredSource?.safetyStock ?? 0))));
    setRequestState('IDLE');
    setRequestOpen(true);
  };
        ...
        <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5">
          <div className="min-w-0">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              {selectedChannel && <span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${CHANNEL_TYPE_META[selectedChannel.channelType].className}`}>현재 조회 · {CHANNEL_TYPE_META[selectedChannel.channelType].label}</span>}
            ...
            <aside className="space-y-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-4 space-y-2 border-t border-slate-100 pt-4 text-xs">
                  ...
                  <div className="flex items-center justify-between"><span className="text-slate-500">현재 조회 판매처</span><strong className="max-w-[150px] text-right text-slate-800">{selectedChannel?.channelName ?? '통합'}</strong></div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 border-t border-slate-100 pt-3 text-[11px] text-slate-500">동일 SKU 판매처 통합 관리 대상</div>
              </div>
            </aside>

            <div className="min-w-0 space-y-4">
              <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
                <SummaryCard icon={<PackageCheck className="h-4 w-4 text-[#0F4C3A]" />} label={selectedChannel ? '선택 판매처 현재고' : '통합 현재고'} value={`${(selectedChannel?.stock ?? selectedSku.stock).toLocaleString()}${selectedSku.unit}`} />
                <SummaryCard icon={<CheckCircle2 className="h-4 w-4 text-emerald-600" />} label="판매 가능" value={`${(selectedChannel?.availableStock ?? selectedSku.availableStock).toLocaleString()}${selectedSku.unit}`} tone="success" />
                <SummaryCard icon={<Clock3 className="h-4 w-4 text-slate-500" />} label="출고 예정" value={`${(selectedChannel?.outboundScheduled ?? selectedSku.reservedStock).toLocaleString()}${selectedSku.unit}`} />
              ...
              <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="flex overflow-x-auto border-b border-slate-200 px-4 pt-2">
                  ...
                  <TabButton active={activeTab === 'CHANNELS'} onClick={() => setActiveTab('CHANNELS')}>다른 판매처 재고</TabButton>
                ...
                {activeTab === 'CHANNELS' && <div className="space-y-4 p-4">
                  <div className="overflow-hidden rounded-xl border border-slate-200">
                    <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold text-slate-950">다른 판매처 동일 SKU 재고</p><p className="mt-1 text-[10px] text-slate-500">안전재고를 제외한 이동 가능 수량을 비교한 뒤, 재고를 보낼 판매처를 선택합니다.</p></div><div className="flex items-center gap-3"><span className="text-[10px] font-bold text-slate-500">{channelRows.length}개 판매처</span><button type="button" onClick={openTransferRequest} disabled={!requestSourceId} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl border border-[#0F4C3A] bg-white px-3.5 py-2 text-[11px] font-bold text-[#0F4C3A] transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-300 disabled:hover:bg-white"><Truck className="h-3.5 w-3.5" />선택 판매처에서 재고 요청</button></div></div>
                      ...
                      <table className="w-full min-w-[1240px] text-left text-xs">
                        <thead className="bg-white text-[10px] font-bold text-slate-400">
                          <tr>
                            ...
                            <th className="px-3 py-3 text-right"><InlineTooltip label="이동 가능" text="이동 가능 수량 = 판매 가능 재고 - 안전재고 - 이동 요청 중 수량. 현재 시연 데이터에서는 이동 요청 중 수량을 0으로 계산합니다." /></th>
                            <th className="px-3 py-3 text-right">일평균 판매</th>
                            <th className="px-3 py-3 text-right">예상 소진</th>
                            <th className="px-3 py-3">재고 수급</th>
                            <th className="px-3 py-3">이동 적합</th>
                        ...
                        <tbody>{channelRows.map((row) => {
                          ...
                          const isSource = row.id === requestSourceId;
                          const movableStock = Math.max(0, row.availableStock - row.safetyStock);
                          const stockoutDays = getDaysToStockout(row);
                          const suitability = isCurrent
                            ...
                            : movableStock <= 0
                              ...
                              : row.health === 'SURPLUS' || (stockoutDays ?? 0) >= 45
                                ? { label: '추천', className: 'border-emerald-200 bg-emerald-50 text-emerald-700' }
                                : (stockoutDays ?? 0) >= 30
                                  ? { label: '검토', className: 'border-amber-200 bg-amber-50 text-amber-700' }
                                  : { label: '비추천', className: 'border-slate-200 bg-slate-50 text-slate-500' };
                          return <tr key={row.id} onClick={() => { if (!isCurrent) setRequestSourceId(row.id); }} className={`border-t border-slate-100 transition ${isCurrent ? 'bg-emerald-50/70' : `cursor-pointer ${isSource ? 'bg-sky-50/80' : 'hover:bg-slate-50'}`}`}>
                            <td className="px-4 py-3"><p className="font-bold text-slate-900">{row.channelName}</p><p className="mt-1 flex items-center gap-1 text-[10px] text-slate-400"><MapPin className="h-3 w-3" />{row.region} · {row.fulfillmentCenter}</p></td>
                            <td className="px-3 py-3"><span className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold ${CHANNEL_TYPE_META[row.channelType].className}`}>{channelIcon(row.channelType)}{CHANNEL_TYPE_META[row.channelType].label}</span></td>
                            ...
                            <td className="px-3 py-3 text-right font-semibold tabular-nums text-slate-700">{row.dailySales.toFixed(1)}{selectedSku.unit}</td>
                            ...
                            <td className="px-4 py-3 text-right"><span className={`text-[10px] font-bold ${isCurrent || isSource ? 'text-[#0F4C3A]' : 'text-slate-400'}`}>{isCurrent ? '현재 조회' : isSource ? '선택됨' : '선택'}</span></td>
                          </tr>;
                        })}</tbody>
                      </table>
                    </div>
                ...
                {activeTab === 'FORECAST' && <div className="space-y-4 p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-4"><InfoValue label="통합 일평균 판매" value={`${totalDailySales.toFixed(1)}${selectedSku.unit}`} /><InfoValue label="14일 예상수요" value={`${totalForecast}${selectedSku.unit}`} /><InfoValue label="판매처 통합 재고" value={`${salesStock}${selectedSku.unit}`} /><InfoValue label="선택 판매처 최근 LOT" value={nearestExpiryLot?.expiryLabel ?? selectedSku.expiryLabel} highlight /></div>
                  <div className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between"><div><p className="text-xs font-bold text-slate-900">현재 판매속도 유지 시 재고 추이</p><p className="mt-1 text-[10px] text-slate-400">판매처 통합 수요 기준 · 시연용 예상값</p></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${risk.className}`}>{risk.label}</span></div><div className="mt-4 h-64 w-full"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={forecastData} margin={{ top: 12, right: 10, left: -20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="label" fontSize={10} stroke="#64748b" /><YAxis yAxisId="stock" fontSize={10} stroke="#e11d48" /><YAxis yAxisId="loss" orientation="right" fontSize={10} stroke="#d97706" unit="만" /><Tooltip contentStyle={{ borderRadius: 10, borderColor: '#cbd5e1', fontSize: 11 }} /><Area yAxisId="stock" type="monotone" dataKey="remaining" fill="#ffe4e6" stroke="#e11d48" strokeWidth={2.5} name="예상 잔여재고" /><Line yAxisId="loss" type="monotone" dataKey="accumulatedLoss" stroke="#d97706" strokeWidth={2.5} strokeDasharray="5 4" dot={false} name="누적 보관손실" /><ReferenceLine yAxisId="stock" x={`D+${expiryDay}`} stroke="#be123c" strokeDasharray="3 3" /></ComposedChart></ResponsiveContainer></div></div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold text-[#0F4C3A]">위험 판단 및 추천 근거</p><p className="mt-2 text-xs leading-5 text-slate-700">{selectedSku.riskReason} 판매처별로는 과잉재고와 부족재고가 동시에 발생해, 추가 생산보다 재할당 또는 RT 검토가 우선입니다.</p></div>
                </div>}

                {activeTab === 'LOTS' && <div className="space-y-3 p-4">
                  <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-slate-950">선택 판매처 LOT별 재고</p><p className="mt-1 text-[11px] text-slate-500"><strong className="text-slate-700">{selectedChannel.channelName}</strong>에 할당된 재고를 LOT별로 확인하고 FEFO 출고순서를 적용합니다.</p></div><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">{selectedChannelLots.length}개 LOT</span></div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="transfer-request-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setRequestOpen(false); }}>
          <section className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4">
              <div><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-50 text-[#0F4C3A]"><Truck className="h-4 w-4" /></span><div><p className="text-[10px] font-bold text-emerald-700">담당자 직접 요청</p><h3 id="transfer-request-title" className="text-lg font-black text-slate-950">재고 재할당·RT 요청</h3></div></div><p className="mt-2 text-xs text-slate-500">AI 전략을 생성하지 않고 동일 SKU의 여유재고를 직접 요청합니다.</p></div>
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="grid gap-3 sm:grid-cols-[1fr_auto_1fr] sm:items-end">
                <label className="text-xs font-bold text-slate-700">보내는 판매처<select value={requestSource.id} onChange={(event) => { const next = sourceCandidates.find((row) => row.id === event.target.value); setRequestSourceId(event.target.value); setRequestQuantity(Math.min(20, Math.max(1, (next?.availableStock ?? 1) - (next?.safetyStock ?? 0)))); setRequestState('IDLE'); }} className="mt-2 h-12 w-full rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold outline-none focus:border-[#0F4C3A]">{sourceCandidates.map((row) => <option key={row.id} value={row.id}>{row.channelName} · 이동 가능 {Math.max(0, row.availableStock - row.safetyStock)}{selectedSku.unit}</option>)}</select></label>
                <div className="flex h-12 items-center justify-center text-xs font-black text-[#0F4C3A]">{requestMode === '재고 재할당' ? '재할당' : 'RT'} →</div>
              ...
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  <ValidationItem label="보내는 판매처 안전재고" value={`이동 후 ${requestSource.availableStock - validatedQuantity}${selectedSku.unit} · 기준 ${requestSource.safetyStock}${selectedSku.unit}`} valid={requestSource.availableStock - validatedQuantity >= requestSource.safetyStock} />
