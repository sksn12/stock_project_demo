...
const NAV_ITEMS: { name: string; href: string; icon: any; badge?: string }[] = [
  ...
  { name: "통합 재고조회", href: "/inventory/all", icon: Boxes },
  { name: "AI 전략 및 시뮬레이션", href: "/strategy/history", icon: History },
  {
    name: "AI 전략 기록 & 성과",
    ...
    <aside
      ...
      className={`fixed inset-y-0 left-0 flex h-screen flex-col justify-between bg-white border-r border-slate-200 select-none shadow-sm transition-[width,transform] duration-200 ease-out md:sticky md:top-0 md:z-30 md:translate-x-0 ${
        isOpen
          ...
          : "w-72 -translate-x-full md:w-[76px]"
      } z-50 shrink-0`}
    >
      <div>
        {/* Brand Header */}
        <div
          className={`flex items-center border-b border-slate-100 ${isOpen ? "justify-start p-5" : "justify-center p-3"}`}
        >
          <div
          ...
          >
            ...
            <div className={isOpen ? "block" : "hidden"}>
              <h1 className="font-bold text-sm tracking-tight text-slate-900 flex items-center gap-1.5">
                HYUNDAI{" "}
                <span className="text-[#9E7C3B] text-xs font-semibold">
                  {isInventoryArea ? "GROUP" : "SEOUL"}
                </span>
              </h1>
        ...
        {/* Department Info */}
        <div
          className={`mx-3 my-3 rounded-lg bg-emerald-50/60 border border-emerald-100 flex items-center text-xs text-slate-700 ${isOpen ? "gap-2.5 p-2.5" : "justify-center p-2"}`}
          title={
            isOpen
              ? undefined
              : isInventoryArea
        ...
        >
          ...
          <div className={isOpen ? "overflow-hidden" : "hidden"}>
            <p className="font-semibold text-[#0F4C3A] truncate">
              {isInventoryArea
                ? "판매처 통합재고 관제"
                : "더현대 서울 재고전략팀"}
            </p>
        ...
        <nav className="px-3 space-y-1">
          <div
            className={`px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 ${isOpen ? "block" : "hidden"}`}
          >
      ...
      <div
        className={`m-3 rounded-lg bg-slate-50 border border-slate-200 text-[11px] text-slate-500 flex items-center ${isOpen ? "gap-2 p-3" : "justify-center p-2"}`}
        title={
          isOpen
            ? undefined
            : isInventoryArea
      ...
      >
        ...
        <div className={isOpen ? "block" : "hidden"}>
          <p className="text-slate-800 font-semibold text-[11px]">
            {isInventoryArea ? "판매처 통합 데이터" : "더현대 서울 전용 관제"}
          </p>
