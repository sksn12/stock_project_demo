...
function UnifiedInventoryContent() {
  ...
  const regions = useMemo(() => [...new Set(inventoryRows.map((item) => item.region).filter((item) => item !== '전국'))].sort(), [inventoryRows]);
  const salesPoints = useMemo(() => [...new Set(inventoryRows.filter((item) => scope === 'ALL' || item.channelType === scope).map((item) => item.channelName))].sort(), [inventoryRows, scope]);
  const stockLocations = useMemo(() => [...new Set(inventoryRows
    .filter((item) => locationScope === 'ALL' || getInventoryLocationType(item) === locationScope)
    .map(getInventoryLocationName))].sort(), [inventoryRows, locationScope]);
  ...
  const listItems = useMemo(() => {
    ...
    const base: InventoryListItem[] = skuPairs.flatMap(({ product, sku }) => getChannelInventoryBySku(sku.id)
      .filter((row) => (scope === 'ALL' || row.channelType === scope)
        && (locationScope === 'ALL' || getInventoryLocationType(row) === locationScope))
    ...
    return base.filter(({ product, sku, channel }) => {
      ...
      if (stockLocation !== 'ALL' && getInventoryLocationName(channel) !== stockLocation) return false;
  ...
  }, [categoryLarge, categoryMedium, categorySmall, locationScope, query, region, riskFilter, salesPoint, scope, skuPairs, stockLocation]);

  const locationCards = useMemo(() => {
    const scopes: LocationFilter[] = ['ALL', 'CENTER', 'STORE'];
    return scopes.map((item) => {
      const rows = item === 'ALL' ? inventoryRows : inventoryRows.filter((row) => getInventoryLocationType(row) === item);
      return {
        key: item,
        label: item === 'ALL' ? '전체 재고 위치' : item === 'CENTER' ? '물류센터' : '오프라인 매장',
        description: item === 'ALL' ? '물류센터·매장 실물재고' : item === 'CENTER' ? '온라인 할당·공용재고' : '점포 직접 보유재고',
      ...
      <section className="space-y-3">
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-2xl font-bold tracking-tight text-slate-950">현대그린푸드 재고 네트워크</h1>
              ...
              <p className="mt-1.5 max-w-3xl text-xs leading-5 text-slate-500">판매 채널의 수요와 물류센터·매장의 실제 보관재고를 분리해 보고, 추가 생산 전에 보충·재할당·RT 가능성을 확인합니다.</p>
            </div>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {locationCards.map((item) => {
            ...
            return (
              <button type="button" key={item.key} onClick={() => { setLocationScope(item.key); setStockLocation('ALL'); }} className={`rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${isActive ? 'border-[#0F4C3A] bg-[#F0F7F4] ring-2 ring-[#0F4C3A]/10' : 'border-slate-200 bg-white'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2.5"><span className={`flex h-9 w-9 items-center justify-center rounded-xl ${isActive ? 'bg-[#0F4C3A] text-white' : 'bg-slate-100 text-slate-500'}`}>{item.key === 'STORE' ? <Store className="h-4 w-4" /> : item.key === 'CENTER' ? <Warehouse className="h-4 w-4" /> : <Database className="h-4 w-4" />}</span><div><p className="text-sm font-bold text-slate-950">{item.label}</p><p className="mt-0.5 text-[10px] text-slate-500">{item.description}</p></div></div>
                ...
                <div className="mt-4 grid grid-cols-4 divide-x divide-slate-200/70 rounded-xl bg-white/70 py-2.5 text-center">
                  ...
                  <div><p className="text-[9px] text-slate-400">출고 예정</p><p className="mt-1 text-sm font-bold tabular-nums text-slate-700">{item.outbound.toLocaleString()}</p></div>
                  <div><p className="text-[9px] text-slate-400">위험·주의</p><p className="mt-1 text-sm font-bold tabular-nums text-rose-600">{item.risky}</p></div>
                </div>
              </button>
            );
          })}
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 bg-slate-50/70 p-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="shrink-0">
            <p className="text-sm font-extrabold text-slate-900">판매 채널·재고 위치별 운영재고</p>
            <p className="mt-1 text-[10px] text-slate-500">판매 채널과 실제 보관 위치를 구분해 SKU별 위험재고를 확인합니다.</p>
          </div>
          <div className="flex flex-1 flex-wrap items-center gap-2 xl:justify-end">
            <div className="relative min-w-[240px] flex-1 xl:max-w-sm">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="상품명, SKU, 판매처, 재고 위치 검색" className="h-10 w-full rounded-xl border border-slate-300 bg-white pl-9 pr-9 text-xs outline-none transition focus:border-[#0F4C3A] focus:ring-2 focus:ring-[#0F4C3A]/10" />
            ...
            <label className="relative w-[145px] shrink-0"><span className="sr-only">판매 채널</span><select value={scope} onChange={(event) => { setScope(event.target.value as ScopeFilter); setSalesPoint('ALL'); }} className="h-10 w-full appearance-none rounded-xl border border-slate-300 bg-white py-0 pl-3 pr-9 text-xs font-semibold text-slate-700 outline-none focus:border-[#0F4C3A]"><option value="ALL">전체 판매채널</option><option value="ONLINE">온라인</option><option value="OFFLINE">오프라인</option><option value="CENTER">공용 미할당</option></select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /></label>
            ...
            <FilterSelect label="재고 위치" value={stockLocation} onChange={setStockLocation} options={stockLocations} allLabel="전체 재고 위치" width="w-[210px]" />
                ...
                {listItems.map(({ product, sku, channel }) => {
                  ...
                  return (
                    <tr key={rowKey} tabIndex={0} role="button" onClick={() => setSelectedItem({ product, sku, channel })} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedItem({ product, sku, channel }); }} className={`group cursor-pointer border-b border-slate-100 outline-none transition hover:bg-emerald-50/40 focus:bg-emerald-50/60 ${bundleSelected ? 'bg-emerald-50/70' : ''}`}>
                      ...
                      <td className="px-4 py-3.5"><div><span className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-bold ${CHANNEL_TYPE_META[channel.channelType].className}`}>{scopeIcon(channel.channelType)}{CHANNEL_TYPE_META[channel.channelType].label}</span><p className="mt-1.5 max-w-[190px] font-bold text-slate-800">{getInventoryAllocationLabel(channel)}</p><p className="mt-1 text-[10px] text-slate-400">{channel.channelName}</p></div></td>
                      <td className="px-4 py-3.5"><div className="flex items-start gap-2"><span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${INVENTORY_LOCATION_META[getInventoryLocationType(channel)].className}`}>{getInventoryLocationType(channel) === 'CENTER' ? <Warehouse className="h-3.5 w-3.5" /> : <Store className="h-3.5 w-3.5" />}</span><div><p className="font-bold text-slate-800">{getInventoryLocationName(channel)}</p><p className="mt-1 flex items-center gap-1 text-[10px] text-slate-400"><MapPin className="h-3 w-3" />{channel.region} · {INVENTORY_LOCATION_META[getInventoryLocationType(channel)].label}</p>{channel.channelType === 'OFFLINE' && <p className="mt-1 text-[9px] text-indigo-500">보충: {channel.fulfillmentCenter}</p>}</div></div></td>
                      <td className="px-4 py-3.5"><div className="flex items-center gap-3"><SkuThumbnail src={SKU_OPERATION_DATA[sku.id]?.imageUrl ?? product.imageUrl} alt={sku.optionLabel} /><div className="min-w-0"><p className="text-[13px] font-extrabold text-slate-950">{sku.optionLabel}</p><p className="mt-1 text-[10px] font-semibold text-slate-600">{product.name}</p><p className="mt-1 font-mono text-[10px] text-slate-400">{sku.code} · {product.category}</p><div className="mt-1.5 flex gap-1">{Object.entries(sku.options).slice(0, 3).map(([key, value]) => <span key={key} className="rounded bg-slate-100 px-1.5 py-0.5 text-[9px] text-slate-500">{key} {value}</span>)}</div></div></div></td>
                      ...
                      <td className="px-4 py-3.5 text-right font-semibold tabular-nums text-slate-600">{channel.safetyStock.toLocaleString()}{sku.unit}</td>
                      ...
                      <td className="px-5 py-3.5 text-right"><span className="inline-flex items-center gap-1 text-[11px] font-bold text-[#0F4C3A]">재고 상세<ArrowRight className="h-3.5 w-3.5" /></span></td>
                    </tr>
                  );
                })}
              </tbody>
